...
The LEDC peripheral lets you configure both **frequency** and **resolution** per channel. In this lab you will use `ledcAttach(pin, freq, resolution)` to set up a channel and `ledcWrite(pin, duty)` to control the duty cycle.
...
For example, servo control needs 50 Hz and fine angle control, so 16-bit works well ($50 \times 65536 = 3.3\text{M}$, well under 80M). But 16-bit at 5 kHz would require $5000 \times 65536 = 327\text{M}$ clock ticks per second -- impossible with an 80 MHz clock. If you try, `ledcAttach` will silently reduce the resolution to fit.
...
This is possible because the LEDC is a **hardware peripheral** -- dedicated circuitry on the chip that generates PWM signals independently of the CPU. Once you call `ledcAttach` and `ledcWrite`, the hardware takes over. The CPU does not spend any time toggling pins; it is free to do other work.
